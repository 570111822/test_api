...
class ceshi(unittest.TestCase):  # 定义一个类，类的首字母要大写哦
    ...
    def test_put1(self):  # 定义一个方法，切记要以test开头哦
        u'''用户登录接口'''   #接口添加注释
        self.url = 'http://tdr-authstru.intra.sit.beyonds.gw/v1/org/update'
        data = {"id":123,"orgLevelId":123456,"orgName": "丙晟科技","parentId": 123}
        ...
        r = requests.put(self.url,json=data, headers=headers)  # 传入参数
        result = r.json()
        # result = json.loads(r.text)  # 使用json格式返回
        self.assertEqual(result['message'], '成功','测试成功')  # 检验返回值，断言是否相等
...
#
#     # 测试报告路径
#     report_path = r"C:\Users\dongs\Desktop\report.html"
